...
<x-layout>
    <div class="container mx-auto p-4">
        ...
        @if (Context::get('profile') === null)
            <div class="bg-yellow-100 dark:bg-yellow-800 text-yellow-700 dark:text-yellow-200 p-4 rounded mb-4">
                {{ __('No profiles found nor selected. Please create a profile and select it to get started.') }}
            </div>
        @else
            <!-- Navigation Options -->
            <nav class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <a href="{{ route('health.metrics.index') }}"
                   class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-4 px-6 rounded-lg text-center transition duration-200 flex gap-4 items-center justify-center">
                    <svg viewBox="0 0 512 512" class="size-8" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                            <g id="add" fill="#fff" transform="translate(42.666667, 64.000000)">
